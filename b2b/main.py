...
async def log_partner_call(
    ...
    row = await pool.fetchrow(
        ...
        conversation_id,
        message_id,
        json.dumps(partner_payload, cls=DecimalEncoder) if partner_payload is not None else None,
        json.dumps(response_payload, cls=DecimalEncoder) if response_payload is not None else None,
        endpoint,
    )
