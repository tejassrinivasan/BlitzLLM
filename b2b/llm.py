...
async def fetch_upcoming_data(calls: List[dict], keys: List[str], constraints: dict = None):
    ...
    async with httpx.AsyncClient() as client:
        for call in calls:
                ...
                for date_str in dates_to_try:
                    ...
                    if "/players/" in url:
                        ...
                        if not player_id:
                            ...
                            break
                        formatted_url = url.format(date=date_str, playerid=player_id, **{k: v for k, v in dated_params.items() if k not in ["playerid", "player", "date"]})
                    else:
                        formatted_url = url.format(date=date_str, **{k: v for k, v in dated_params.items() if k != "date"})
            ...
            print(f"[DEBUG] Results: {json.dumps(results, cls=DecimalEncoder, indent=2)}")
...
async def generate_text_response(
    ...
    try:
        ...
        return {
            ...
            "postgresqlResults": json.loads(json.dumps(historical_results, cls=DecimalEncoder)) if historical_results else None,  # Include the historical results (can be None)
            "upcomingResults": json.loads(json.dumps(live_data, cls=DecimalEncoder)) if live_data else None # Include live data (can be None)
        }
