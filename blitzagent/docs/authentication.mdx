...
async def test_api_key():
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(
            "https://blitzagent.onrender.com/health",
            headers=headers
        )
...
<AccordionGroup>
  ...
  <Accordion icon="shield" title="Request Security">
    **HTTPS Only**:
    - Always use HTTPS endpoints (`https://blitzagent.onrender.com`)
...
class BlitzClient:
    def __init__(self, api_key: Optional[str] = None, base_url: str = "https://blitzagent.onrender.com"):
...
class BlitzClient {
    constructor(apiKey = process.env.BLITZ_API_KEY, baseUrl = 'https://blitzagent.onrender.com') {
        if (!apiKey) {
