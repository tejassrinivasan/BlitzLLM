...
BlitzAgent's streaming analysis feature provides real-time insights into the analysis process, allowing you to see exactly how complex sports queries are being processed. This is particularly valuable for understanding the reasoning behind analysis results and monitoring progress on time-intensive queries.

## How Streaming Works

When you use the `/analyze/stream` endpoint, BlitzAgent provides a Server-Sent Events (SSE) stream that shows:
...
<Steps>
  ...
  <Step title="Data Retrieval">
    Real-time updates as BlitzAgent fetches data from various sources including databases, web scraping, and betting APIs.
  </Step>
  
  <Step title="AI Reasoning">
    Live updates showing Claude 4 Sonnet's reasoning process as it analyzes the data and formulates insights.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion icon="search" title="league_detection">
    **When it occurs**: When BlitzAgent determines which sport/league the query refers to
  ...
  <Accordion icon="brain" title="reasoning">
    **When it occurs**: During Claude's analysis and reasoning phases
