...
The `/health` endpoint provides a quick way to check if BlitzAgent is running properly and if all required services are available.
...
<ResponseField name="version" type="string" optional>
  BlitzAgent version information
</ResponseField>
...
class BlitzAgentMonitor:
    def __init__(self, base_url: str = "http://localhost:8001"):
        self.base_url = base_url
        self.logger = logging.getLogger(__name__)
    
    async def check_health(self) -> dict:
        """Check BlitzAgent health status"""
        try:
            async with httpx.AsyncClient() as client:
    ...
    async def monitor_continuously(self, interval: int = 60):
        ...
        while True:
            ...
            if status == "healthy":
                self.logger.info("✅ BlitzAgent is healthy")
            elif status == "degraded":
                self.logger.warning("⚠️ BlitzAgent is degraded")
            else:
                self.logger.error("❌ BlitzAgent is unhealthy")
...
class BlitzAgentMonitor {
  ...
  async monitorContinuously(interval = 60000) {
    setInterval(async () => {
      ...
      switch (status) {
        case 'healthy':
          console.log('✅ BlitzAgent is healthy');
          break;
        case 'degraded':
          console.warn('⚠️ BlitzAgent is degraded');
          break;
        default:
          console.error('❌ BlitzAgent is unhealthy');
          // Send alert
...
// Usage
const monitor = new BlitzAgentMonitor();
...
<AccordionGroup>
  ...
  <Accordion icon="server" title="API Server Issues">
    ...
    **Solutions:**
    1. Check if BlitzAgent process is running
...
<CardGroup cols={2}>
  <Card
    ...
    href="/configuration"
  >
    Learn how to configure BlitzAgent for optimal health
  </Card>
