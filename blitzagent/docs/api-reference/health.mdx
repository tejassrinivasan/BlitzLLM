...
api: "GET https://api.blitz.com/health"
---
...
const response = await fetch('https://api.blitz.com/health');
...
async def check_health():
    async with httpx.AsyncClient() as client:
        response = await client.get("https://api.blitz.com/health")
...
services:
  blitzagent:
    ...
    ports:
      - "8001:8001"
    healthcheck:
      test: ["CMD", "curl", "-f", "https://api.blitz.com/health"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 40s
...
class BlitzMonitor:
    def __init__(self, base_url: str = "https://api.blitz.com"):
...
class BlitzMonitor {
  constructor(baseUrl = 'https://api.blitz.com') {
