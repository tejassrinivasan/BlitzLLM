...
The `/analyze/stream` endpoint provides real-time streaming of analysis events, allowing you to see BlitzAgent's reasoning process as it happens. This is particularly useful for complex queries that may take time to process.
...
const streamAnalysis = () => {
  fetch('http://localhost:8001/analyze/stream', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer YOUR_API_KEY'
...
<AccordionGroup>
  ...
  <Accordion icon="target" title="league_detection">
    Shows when BlitzAgent has automatically detected the sport/league.
...
BlitzAgent includes a built-in stream viewer for testing the streaming endpoint interactively:

<Steps>
  <Step title="Start BlitzAgent Server">
    Make sure your BlitzAgent server is running:
    ```bash
    python main.py
    ```
  </Step>
  ...
  <Step title="Open Stream Viewer">
    ...
    ```
    http://localhost:8001/stream_viewer.html
    ```
    
    Or open the `stream_viewer.html` file directly from your BlitzAgent installation directory.
  </Step>
