...
import { readFileSync, createReadStream, lstatSync } from 'fs';
...
class ConfigManager {
  ...
  loadConfig() {
    try {
      ...
      console.log("\u{1F4C1} Configuration loaded successfully");
  ...
  logEnabledServices() {
    ...
    if (this.config.services.firecrawl.enabled) enabledServices.push("\u{1F577}\uFE0F  Firecrawl (web scraping)");
    if (this.config.services.sportsdata.enabled) enabledServices.push("\u26BE SportsData APIs");
    if (this.config.services.azure.search.enabled) enabledServices.push("\u{1F50D} Azure Search");
    if (this.config.services.azure.openai.enabled) enabledServices.push("\u{1F9E0} Azure OpenAI");
    if (this.config.services.google.gemini.enabled) enabledServices.push("\u{1F48E} Google Gemini");
    if (this.config.services.anthropic.enabled) enabledServices.push("\u{1F916} Anthropic Claude");
    if (this.config.services.postgres.enabled) enabledServices.push("\u{1F5C4}\uFE0F  PostgreSQL");
    if (this.config.services.cosmosdb.enabled) enabledServices.push("\u{1F30C} Cosmos DB");
  ...
  isServiceEnabled(service) {
    ...
    let current = this.config.services;
...
const postgresConfig$1 = configManager.getServiceConfig("postgres");
const azureConfig$1 = configManager.getServiceConfig("azure.openai");
if (azureConfig$1 && azureConfig$1.enabled) {
  process.env.AZURE_API_KEY = azureConfig$1.apiKey;
  process.env.AZURE_RESOURCE_NAME = azureConfig$1.resourceName;
  process.env.AZURE_OPENAI_ENDPOINT = azureConfig$1.endpoint;
  process.env.AZURE_OPENAI_API_VERSION = azureConfig$1.apiVersion;
  console.log("Azure OpenAI config loaded:");
  console.log("Resource Name:", azureConfig$1.resourceName);
  console.log("API Version:", azureConfig$1.apiVersion);
  console.log("Endpoint:", azureConfig$1.endpoint);
} else {
  throw new Error("Azure OpenAI configuration is required. Please configure azure.openai in your config.");
}
const connectionString$1 = `postgresql://${postgresConfig$1.user}:${encodeURIComponent(postgresConfig$1.password)}@${postgresConfig$1.host}:${postgresConfig$1.port}/agentmemory`;
console.log("\u{1F916} Using Azure OpenAI model: gpt-4o");
...
const memory$1 = new Memory({
  ...
  vector: new PgVector({
    ...
    pgPoolOptions: {
      ssl: postgresConfig$1.ssl ? { rejectUnauthorized: false } : false
    }
  }),
  embedder,
  options: {
...
console.log("\u{1F9E0} Memory Configuration:");
...
console.log("\u{1F527} Loading MCP tools from Python server...");
const mcpTools$1 = await mcp.getTools();
console.log("\u2705 MCP tools loaded:", Object.keys(mcpTools$1));
console.log("\u{1F50D} Tool Schemas Debug:");
Object.entries(mcpTools$1).slice(0, 3).forEach(([toolName, tool]) => {
  console.log(`Tool: ${toolName}`);
  console.log(`Description: ${tool.description}`);
  console.log(`Schema:`, JSON.stringify(tool.schema, null, 2));
  console.log("---");
});
const sportsInsightsAgent = new Agent({
  ...
  Current time: ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}  
  Current year: ${(/* @__PURE__ */ new Date()).getFullYear()}
  ...
  **Quality Assurance:**
  ...
  defaultStreamOptions: {
    maxRetries: 3,
    maxSteps: 25,
    // Increased for enhanced workflow
    temperature: 0.1,
    // Slightly higher for more natural thinking-out-loud
    maxTokens: 16384
...
const CONSISTENT_MODEL_PARAMS = {
  temperature: 0.3,
  // Slightly higher temperature for creative question generation
  maxRetries: 3,
  maxSteps: 100,
  // Increased from 50 to allow for multiple complete query cycles
  maxTokens: 16384
};
console.log("\u{1F527} Loading MCP tools for Generate Queries Agent...");
const mcpTools = await mcp.getTools();
console.log("\u2705 MCP tools loaded for Generate Queries Agent:", Object.keys(mcpTools));
const generateQueriesAgent = new Agent({
...
Current year is ${(/* @__PURE__ */ new Date()).getFullYear()}. Current date is ${(/* @__PURE__ */ new Date()).toLocaleDateString()}.
...
Be thorough, accurate, and creative in your query generation.`,
  ...
  memory,
  tools: mcpTools,
  // Use all MCP tools from Python server
